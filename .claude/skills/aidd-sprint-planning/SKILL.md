...
1. Cubre, como minimo: **duracion del sprint**, **capacidad** (nº de personas/perfiles disponibles por sprint, segun el plan de recursos), **fecha de inicio** y **velocity asumida** (si no hay historica, propon una y marcala como supuesto).
...
6. Modo no interactivo: toma el default recomendado para `preferencia` y `confirmacion` (p. ej. sprint de 2 semanas); deja los `bloqueante` sin default como supuestos en el documento.
...
- Duracion de sprint, capacidad por sprint, fecha de inicio, velocity asumida y recursos/equipo de referencia.

## 2. Unidades de trabajo
- Tabla: id (change/HU), descripcion breve, fase (F0/F1/F2), estimacion (S/M/L o puntos), perfil principal.
...
- Por cada sprint: objetivo, unidades incluidas, estimacion agregada vs capacidad, asignacion de perfiles y Definition of Done. Comprueba que ninguna unidad va antes que sus prerequisitos.
